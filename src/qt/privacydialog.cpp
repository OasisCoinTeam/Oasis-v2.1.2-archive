                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zOASIS ought to be enough for anybody." - Bill Gates, 2017
    ui->zOASISpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOASISSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzOASIS->setEnabled(false);
        ui->pushButtonMintzOASIS->setToolTip(tr("zOASIS is currently disabled due to maintenance."));

        ui->pushButtonSpendzOASIS->setEnabled(false);
        ui->pushButtonSpendzOASIS->setToolTip(tr("zOASIS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOASISpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzOASIS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOASIS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOASIS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOASIS in ") +
...
void PrivacyDialog::on_pushButtonSpendzOASIS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOASIS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zOASIS
        sendzOASIS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOASIS
    sendzOASIS();
}

void PrivacyDialog::on_pushButtonZOASISControl_clicked()
{
    ZOASISControlDialog* zOASISControl = new ZOASISControlDialog(this);
    zOASISControl->setModel(walletModel);
    zOASISControl->exec();
}

void PrivacyDialog::setZOASISControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOASISSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOASIS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Oasis Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOASISpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOASIS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OASIS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOASIS</b>";
    ...
    // use mints from zOASIS selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZOASISControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZOASISControlDialog::GetSelectedMints();
    }
    ...
    ZOASISControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOASIS Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " OASIS, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zOASIS Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOASIS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOASIS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOASIS "));
    ui->labelzOASISAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzOASISSyncStatus->setVisible(fShow);
}
