...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::OASIS);
    ...
    if (!settings.contains("nAnonymizeOasisAmount"))
        settings.setValue("nAnonymizeOasisAmount", 1000);

    nAnonymizeOasisAmount = settings.value("nAnonymizeOasisAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOasisAmount"))
        SoftSetArg("-anonymizeoasisamount", settings.value("nAnonymizeOasisAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in oasis.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOasisAmount:
            return QVariant(nAnonymizeOasisAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOasisAmount:
            ...
            emit anonymizeOasisAmountChanged(nAnonymizeOasisAmount);
            break;
        case CoinControlFeatures:
