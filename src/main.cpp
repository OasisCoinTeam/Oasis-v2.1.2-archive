...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOASIS spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zOASIS spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zOASIS spend with serial %s from tx %s is not in valid range\n",
        ...
        // BIP65 CLTV Activation
        bool fCLTVActive = chainActive.Height() >= Params().CLTV_ActivationBlock();
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight < 1051200 && nHeight > 0) {
        nSubsidy = 0.3 * COIN;
    } else if (nHeight < 3153600 && nHeight > 1051200) {
        nSubsidy = 0.2 * COIN;
    } else if (nHeight < 5256000 && nHeight > 3153600) {
        nSubsidy = 0.15 * COIN;
    } else if (nHeight < 7358400 && nHeight > 5256000) {
        nSubsidy = 0.1 * COIN;
    } else if (nHeight < 9460800 && nHeight > 7358400) {
        nSubsidy = 0.05 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	} else if (nHeight < 9999999 &&  nHeight > 0) {
...
CBitcoinAddress addressExp1("oLckfEeUHGrXcUdeuEaJCKKnkkJQ3MfNQF");
CBitcoinAddress addressExp2("oHpsrBcULZpFiHiUmwC9UapuWFvfik1KGV");
...
{
    RenameThread("oasis-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZOASISMinted()
{
...
void RecalculateZOASISSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zOASIS supply
...
bool RecalculateOASISSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (fCLTVActive) {
        if (block.nVersion >= 5) {
            // CLTV is activated on this block, so now we verify CLTV scripts!
            flags |= SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY;
        } else {
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zOASIS with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zOASISSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert OASIS to zOASIS
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOasis spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOASIS serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOASIS mints and %d zOASIS spends\n", __func__, nMints, nSpends);
